...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = np.concatenate((box, conf, j), axis=1)[np.squeeze(conf) > conf_thres]
...
def image_process(path):
    ...
    img = img[np.newaxis,:,:,:]  # expand for batch dim
...
def onnx_runtime(model_file, image_file):
    ...
    print('inference time: ', time.time() - start1)
    ...
    print('nms time: ', time.time() - start2)
...
def main():
    ...
    onnx_runtime(model_file, image_file)
